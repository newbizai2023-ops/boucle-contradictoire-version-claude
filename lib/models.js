...
// plus juger. Il reste du même éditeur que l'auditeur, faute d'un quatrième fournisseur dans la
// liste blanche : l'indépendance obtenue est celle du modèle, pas encore celle de l'éditeur.
export const MODEL_DEFAULTS = {
  technical: { writer: "~anthropic/claude-opus-latest", auditor: "openai/gpt-5.6-sol", arbiter: "~x-ai/grok-latest", challenger: "~openai/gpt-latest" },
  financial: { writer: "~anthropic/claude-opus-latest", auditor: "openai/gpt-5.6-sol", arbiter: "~x-ai/grok-latest", challenger: "~openai/gpt-latest" },
  legal: { writer: "~anthropic/claude-opus-latest", auditor: "openai/gpt-5.6-sol", arbiter: "~x-ai/grok-latest", challenger: "~openai/gpt-latest" },
  current_research: { writer: "~anthropic/claude-sonnet-latest", auditor: "openai/gpt-5.6-sol", arbiter: "~x-ai/grok-latest", challenger: "~openai/gpt-latest" },
  general_analysis: { writer: "~anthropic/claude-sonnet-latest", auditor: "~openai/gpt-latest", arbiter: "~x-ai/grok-latest", challenger: "openai/gpt-5.6-terra" }
};
...
// Candidats de repli pour le second avis, du plus au moins souhaitable. Aucun n'est le rédacteur
// par défaut d'un domaine : la liste sert précisément à sortir d'une collision.
...
export function selectModels(task, supplied = {}) {
  ...
  if ([models.writer, models.arbiter].includes(models.challenger)) {
    models.challenger = CHALLENGER_FALLBACKS.find(candidat => candidat !== models.writer && candidat !== models.arbiter) ?? models.challenger;
  }
  return models;
}
