...
/** Contradictions à la fois graves et **dont la citation a été retrouvée dans la page**.
 *
 *  Une réfutation reste la parole d'un modèle : elle peut inventer une objection et lui coller une
 *  URL qui répond. Exiger seulement que la page réponde revenait donc à conclure « la contradiction
 *  est confirmée » parce que l'URL existait — accessible n'a jamais valu probant.
 *
 *  Le contrôle porte désormais sur `confirmee`, posé par `validateContradiction` (lib/evidence.js) :
 *  la source figure au dossier, la page a bien été extraite, et l'extrait cité s'y retrouve. Sans
 *  cette validation — Firecrawl désactivé, page injoignable — rien n'est confirmé et rien ne dégrade
 *  le statut : à défaut de pouvoir vérifier, on ne sanctionne pas.
 *
 *  C'est le fait le plus lourd que l'application sache produire contre un document ; il ne doit
 *  dépendre ni de l'appréciation de l'arbitre, ni de la bonne foi du falsificateur. */
export function confirmedContradictions(falsification) {
  if (!falsification) return [];
  return falsification.contradictions.filter(
    contradiction => isSevere(contradiction?.gravite) && contradiction?.confirmee === true
  );
}
