...
export function shouldStopAfterAudit(audit, minScore, { sources = [], document = "" } = {}) {
  const motifs = [];
  ...
  return { stop: motifs.length === 0, motifs, decision, injoignables };
}
...
export function stagnationBetween(previous, current) {
  ...
  if (apres.score > avant.score || apres.severes < avant.severes) return null;
  return { avant, apres };
}
