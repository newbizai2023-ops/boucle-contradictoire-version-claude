...
/** Statut d'exécution correspondant à la décision de l'arbitre.
 *
 *  La comparaison passe par `normalize` : elle était stricte (`decision === "APPROUVE"`), alors que
 *  le contrat JSON demande `APPROUVE` sans accent mais que « approuvé » s'écrit avec. Un arbitre
 *  répondant `APPROUVÉ` — l'orthographe française correcte, et celle qu'emploie le prompt lui-même
 *  quand il rédige en toutes lettres — tombait donc dans la branche par défaut et voyait son
 *  approbation enregistrée comme un rejet. Même classe d'erreur que `isSevere` avant la 1.5.0.
 *
 *  Une décision absente ou inintelligible ne vaut pas approbation : elle reste un rejet, faute de
 *  quoi un modèle qui omet le champ ferait valider n'importe quel document. Le cas est distingué par
 *  `decisionIntelligible` pour que l'appelant puisse le signaler plutôt que de le taire. */
export function arbitrationStatus(decision) {
  const valeur = decisionCanonique(decision);
  if (/^APPROUVE_AVEC_RESERVES?$/.test(valeur)) return "validated_with_reservations";
  if (/^APPROUVE$/.test(valeur)) return "validated";
  return "rejected_by_arbiter";
}

/** Décision réduite à sa forme canonique : sans accent, en majuscules, séparateurs unifiés.
 *
 *  Les séparateurs comptent autant que les accents : le contrat demande
 *  `APPROUVE_AVEC_RESERVES`, mais un modèle qui rédige en français écrit « Approuvé avec réserves »
 *  avec des espaces. Sans cette réduction, cette réponse-là — une approbation explicite — était
 *  enregistrée comme un rejet. */
function decisionCanonique(decision) {
  return normalize(decision).replace(/[^A-Z]+/g, "_").replace(/^_|_$/g, "");
}

/** Vrai si l'arbitre a rendu une décision reconnaissable, quelle qu'elle soit. */
export function decisionIntelligible(decision) {
  return /^(APPROUVE(_AVEC_RESERVES?)?|REJETE)$/.test(decisionCanonique(decision));
}
