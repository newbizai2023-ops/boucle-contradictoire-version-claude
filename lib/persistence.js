...
export function runSummary(result) {
  ...
  return {
    ...
    // Inventaire du dernier cycle : combien d'affirmations portait le document livré, et combien
    // de déterminantes n'ont pas pu être établies. La seconde colonne est celle qui permet de
    // retrouver les analyses parties à l'arbitrage avec un trou dans leur démonstration.
    claimsTotal: (lastAudit?.claims || []).length,
    claimsCriticalUnverified: (lastAudit?.claims || []).filter(claim => claim.critique && claim.statut !== "VERIFIE").length
  };
}
...
/** Une ligne par affirmation inventoriée, à chaque cycle d'audit.
 *
 *  C'est le niveau de traçabilité qui manquait : `run_sources` dit quelles pages ont été contrôlées,
 *  `run_audits` quels scores ont été rendus, mais rien ne reliait une recommandation aux
 *  affirmations qui la portent. Conserver une ligne par cycle, plutôt que le seul état final, est ce
 *  qui permet de voir l'évolution d'un statut d'un cycle à l'autre. */
export function claimRows(runId, audits) {
  return (audits ?? []).flatMap((audit, index) =>
    (audit?.claims ?? []).map(claim => ({
      ...
      affirmation: String(claim?.affirmation ?? ""),
      statut: claim?.statut ?? null,
      critique: claim?.critique === true,
      // Les URL, pas leur contenu : `run_sources` porte déjà l'état de chaque page, la jointure se
      // fait sur l'URL.
